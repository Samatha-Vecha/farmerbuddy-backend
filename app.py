...
def predict_fertilizer():
    ...
    try:
        ...
        if db:
            db.collection("fertilizer_predictions").add({
                "uid": uid,
                "input": data["input"],
...
def predict_crop():
    ...
    try:
        ...
        if db:
            db.collection("crop_recommendation_predictions").add({
                "uid": uid,
...
def history(uid):
    ...
    try:
        fertilizer_docs = db.collection("fertilizer_predictions").where("uid", "==", uid).stream()
        crop_docs = db.collection("crop_name").where("uid", "==", uid).stream()

        history_data = {
            "fertilizer_history": [{"id": doc.id, **doc.to_dict()} for doc in fertilizer_docs],
            "crop_history": [{"id": doc.id, **doc.to_dict()} for doc in crop_docs]
        }
